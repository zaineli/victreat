...
function TreatmentsPage({ searchParams }: { searchParams: { cancer: string, stage: string, mutation: string, age: string, treatments: string, name: string } }) {
  ...
  const cancerText = searchParams.cancer.split('-').map((c) => c.charAt(0).toUpperCase() + c.slice(1)).join(' ');
  const stageText = searchParams.stage;
  const mutationText = searchParams.mutation;
